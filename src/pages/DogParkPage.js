...
<div className="feed">
    {contentFeed.length > 0 ? (
      contentFeed.map((content) => (
        <div key={content.contentId} className={`content-item ${content.isSigned ? 'signed' : ''}`}>
          <h2>{content.title}</h2>
          {content.description && <p>{content.description}</p>}
          {content.isExpiring && <p className="expiring">Expiring on: {new Date(content.expirationDate).toLocaleDateString()}</p>}
          {content.isSigned && <p className="signed">Signed on blockchain</p>}
          {content.mediaType === 'audio' && <audio src={content.contentUrl} controls />}
          {content.mediaType === 'video' && <video src={content.contentUrl} controls />}
