...
const ChatWindow = ({ accountId }) => {
  ...
  const compareDate = new Date(currentDate.getTime() - 24 * 60 * 60 * 1000);

  // Convert the JavaScript Date to Firestore's Timestamp
  const compareTimestamp = Timestamp.fromDate(compareDate);
    ...
    <div>
      ...
      {!isOpen && messages && messages.length > 0 && (
        <div className="last-message">
          <strong>{messages[messages.length - 1].username}</strong>:{" "}
      ...
      {isOpen && (
        <div className={`chat-window ${isFullScreen ? "full-screen" : ""}`}>
          ...
          <small className="chat-note">
            <i>
              <b>Note:</b> This is all off-chain and public to all users on
              sharddog.social only. Have fun and be nice
            </i>
          </small>
            ...
            {messages &&
              messages.map((msg) => (
                <div key={msg.id} className="message">
                  <strong style={{ color: getColor(msg.username) }}>
                    {msg.username}
                  </strong>
                  : {msg.text}
